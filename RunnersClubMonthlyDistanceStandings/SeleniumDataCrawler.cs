...
using RunnersClubMonthlyDistanceStandings.Pages;
...
namespace RunnersClubMonthlyDistanceStandings
{
    public class SeleniumDataCrawler
    {
        ...
        private void GoToStravaAndFetchData()
        {
            try
            {
                var clubPageUrl = UrlBuilder.ClubPage(_configurationWrapper.GetAddressEndpoint(),
                    _configurationWrapper.GetClubId());

                var steps = new LoginPage(_webDriver)
                    .ClickOnLoginLink()
                    .FillInEmailBox(_configurationWrapper.GetUser())
                    .FillInPasswordBox(_configurationWrapper.GetPassword())
                    .ClickOnLoginButton()
        ...
        {
            var urlToGoTo = UrlBuilder.MonthlyActivities(profileUrl, _configurationWrapper.GetMonthlyFilterValue());
